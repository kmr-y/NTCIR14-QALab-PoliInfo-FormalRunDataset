...
#use lib '/Users/sakamoto/.cpan/build/XML-RegExp-0.04-Hv8FC_/lib/';
#use lib '/Users/sakamoto/.cpan/build/XML-DOM-1.44-GpxvkS/lib/';
#use lib '/Users/sakamotokoutarou/.cpan/build/XML-RegExp-0.04-u8OKKW/lib/';
#use lib '/Users/sakamotokoutarou/.cpan/build/XML-DOM-1.45-vq5gq5/lib/';
use XML::DOM;
use DB_File;
use Getopt::Std;
use Encode;
...
sub printResults {
  ...
  if(!defined($opt_t)||$opt_t==1) {
    print encode('utf-8', "$peerID $methodTag Average_R: $ROUGEAverages->{'AvgR'} ");
    print "($opt_c\%-conf.int. $ROUGEAverages->{'CIAvgL_R'} - $ROUGEAverages->{'CIAvgU_R'})\n";
    print encode('utf-8', "$peerID $methodTag Average_P: $ROUGEAverages->{'AvgP'} ");
    print "($opt_c\%-conf.int. $ROUGEAverages->{'CIAvgL_P'} - $ROUGEAverages->{'CIAvgU_P'})\n";
    print encode('utf-8', "$peerID $methodTag Average_F: $ROUGEAverages->{'AvgF'} ");
...
sub computeROUGEX {
  ...
  unless(exists($ROUGEEval->{"Ps"}{$peerID})) {
    ...
    unless(exists($knownMissing{$evalID}{$peerID})) {
      print STDERR encode('utf-8', "\*ROUGE Warning: test instance for peer $peerID does not exist for evaluation $evalID\n");
  ...
  # evaluate peer
  {
    ...
    @results=();
    print STDERR "PEER: \n";
    print STDERR encode('utf-8', $peerPath);
    print STDERR "\n";
...
sub computeNGramScore {
  ...
  #print STDERR $peerText."\n";
  &createNGram($peerText,\%peer_grams,$NSIZE);
  #print STDERR join("|",%peer_grams)."\n";

  #print STDERR encode('utf-8', "***P $peerPath\n");
  #if(defined($peerText)) {
    #print STDERR encode('utf-8', "$peerText\n");
    #print STDERR encode('utf-8', join("|",%peer_grams)."\n");
  #} else {
...
sub ngramScore {
  ...
  if($model_grams->{"_cn_"}!=0) {
    #print STDERR encode('utf-8', $model_grams->{"_cn_"}."\n");
...
sub createNGram {
  ...
  #print STDERR encode('utf-8', $mx_tokens[$i]."\n");
